...
regtoken=$(curl -s https://github.com/agusgans/adminfinder/blob/master/reg | grep -o "$ctoken" )
if [[ $regtoken == "$ctoken" ]]
	then
...
printf "${merah}[!]${NC} [ID : $aid ] [Reward : $getreward] [Reward : 999999] [${merah}Failed${NC}]\n"
fi
}
